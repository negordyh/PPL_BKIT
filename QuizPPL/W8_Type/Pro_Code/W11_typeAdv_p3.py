...
class StaticCheck(Visitor):
    ...
    def visitFuncDecl(self,ctx:FuncDecl,o):
        for x in o:
            if ctx.name==x[0]:
                raise Redeclared(ctx)
        block=[(ctx.name,"func")]        
        block+=reduce(lambda a,b: a+[self.visit(b,a)],ctx.param+ctx.local,[])
        ...
        ctxname=[x.name for x in ctx.param+ctx.local]    
        for x in o:
    ...
    def visitCallStmt(self,ctx:CallStmt,o):
        ...
        for x in o:
            if ctx.name==x[0] :
                args=[self.visit(x,o) for x in ctx.args]
                param=x[1]
                if "" in param:
                    index=param.index("")
